...
class CollapsibleFrame(Frame):
    def __init__(self, master, text=None, borderwidth=2, width=0, height=16, interior_padx=0, interior_pady=8, background=None, caption_separation=8, caption_font=None, caption_builder=None, icon_x=0):
        Frame.__init__(self, master)
        if background is None:
            background = self.cget("background")

        self.configure(background='#5d5d5d')
        ...
        self.interior = Frame(self._containerFrame, background='#ff0000')
...
if __name__ == "__main__":
    ...
    cf1 = CollapsibleFrame(root, text ="Esta es una prueba", interior_padx=5)
    cf1.pack(fill=X)
